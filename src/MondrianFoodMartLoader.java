 ...
 *
 * <p>This is known to create test data for the following databases:
 *
 * <ul>
 *   <li>LucidDB-0.9.2
 *   <li>MySQL 3.23 using MySQL-connector/J 3.0.16
 *   <li>MySQL 4.15 using MySQL-connector/J 3.0.16
 *   <li>Oracle 10g using ojdbc14.jar
 *   <li>Postgres 8.0 beta using postgresql-driver-jdbc3-74-214.jar
 *   <li>FirebirdSQL 1.0.2 and JayBird 1.5 (JDBC)
 * </ul>
 *
 * <p>Output can be to a set of files with create table, insert and create index statements, or
 * directly to a JDBC connection with JDBC batches (lots faster!)
 *
...
public class MondrianFoodMartLoader {
  ...
  public MondrianFoodMartLoader(String[] args) {
    ...
    for (String arg : args) {
      if (arg.equals("-verbose")) {
        ...
        outputBatchSize = Integer.parseInt(arg.substring("-outputJdbcBatchSize=".length()));
      } else {
  ...
  /**
   * Load output from the input, optionally creating tables, populating tables and creating indexes
   */
  ...
  /**
   * Parses a file of INSERT statements and output to the configured JDBC connection or another file
   * in the dialect of the target data source.
   *
   * <p>Assumes that the input INSERT statements are generated by this loader by something like:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * -outputDirectory=C:\Temp\wip\Loader-Output</pre>
   *
   * </blockquote>
   *
   ...
   */
  private void loadDataFromFile(Condition<String> tableFilter, long pauseMillis, int batchSize)
      throws Exception {
    ...
    try {
      ...
      while ((line = bufferedReader.readLine()) != null) {
        ...
        if (!tableName.equals(prevTable)) {
          ...
          quotedColumnNames = columnNames.replaceAll(quoteChar, dialect.getQuoteIdentifierString());
          String[] splitColumnNames =
              columnNames.replaceAll(quoteChar, "").replaceAll(" ", "").split(",");
          ...
          for (int i = 0; i < splitColumnNames.length; i++) {
            ...
            if (thisColumn == null) {
              throw new Exception(
                  "Unknown column in INSERT statement from file: " + splitColumnNames[i]);
            } else {
        ...
        if (infobrightLoad) {
          ...
          fileOutput.write(
              massagedLine.toString().replaceAll("\"", "\\\"").replace('\'', '"').trim());
          fileOutput.write(nl);
        } else {
          massagedLine.setLength(0);
          massagedLine
   ...
   * @param values the contents of the INSERT VALUES clause, for example "34,67.89,'GHt''ab'". These
   *     are in MySQL form.
   ...
   * @throws Exception
   */
  private void loadDataFromJdbcInput(Condition<String> tableFilter, long pauseMillis, int batchSize)
      throws Exception {
    if (outputDirectory != null) {
      file = new File(outputDirectory, "createData.sql");
      fileOutput = new FileWriter(file);
  ...
  private StringBuilder insertSchema(
      StringBuilder sb, String toFind, boolean mandatory, boolean insertBefore) {
  ...
  /**
   * Read the given table from the input RDBMS and output to destination RDBMS or file
   *
   ...
   */
  private int loadTable(String name, Column[] columns, long pauseMillis, int batchSize)
      throws Exception {
    int rowsAdded = 0;
    StringBuilder buf = new StringBuilder();
   ...
   *
   * <p>Otherwise, output the statements to a file.
   *
      ...
      if (sqls.size() == 1) {
        ...
        if (updates < sqls.size()) {
          throw new RuntimeException(
              "Failed to execute batch: " + sqls.size() + " versus " + updates);
        }
      }
      stmt.close();
      if (useTxn) {
  ...
  private void createIndexes(
      boolean baseTables, boolean summaryTables, Condition<String> tableFilter) throws Exception {
    if (outputDirectory != null) {
   ...
   * Creates an index.
   *
   * <p>If we are outputting to JDBC, executes the CREATE INDEX statement; otherwise, outputs the
   * statement to a file.
   */
  private void createIndex(
    ...
    try {
      ...
      buf.setLength(0);
      buf.append(isUnique ? "CREATE UNIQUE INDEX " : "CREATE INDEX ").append(quoteId(indexName));
   ...
   * <p>
   *
   * <p>Also initializes {@link #tableMetadataToLoad} and {@link #aggregateTableMetadataToLoad}.
   *
   ...
   *
   * <p>If we are outputting to JDBC, Execute a DROP TABLE statement Execute the CREATE TABLE
   * statement
   *
   * <p>Otherwise, output the statement to a file.
   */
  private void createTable(String name, Condition<String> tableFilter, Column... columns) {
  ...
  private void createTable(
    ...
    try {
      ...
      for (int i = 0; i < columns.length; i++) {
        ...
        buf.append(nl);
        buf.append("    ").append(quoteId(column.name)).append(" ").append(column.typeName);
  ...
  /** Quote the given SQL identifier suitable for the given DBMS type, with schema. */
  private String quoteId(Dialect dialect, String schemaName, String name) {
    return dialect.quoteIdentifier(schemaName, name);
  }

  /**
   * String representation of the column in the result set, suitable for inclusion in a SQL insert
   * statement.
   *
   * <p>The column in the result set is transformed according to the type in the column parameter.
   *
   * <p>Different DBMSs (and drivers) return different Java types for a given column; {@link
   * ClassCastException}s may occur.
   *
    ...
    if (columnType.startsWith(Type.Integer.name)) {
      ...
      if (columnType.startsWith("TIMESTAMP")) {
        ...
        DecimalFormat formatter =
            new DecimalFormat(decimalFormat(matcher.group(1), matcher.group(2)));
        if (obj.getClass() == Double.class) {
          try {
            Double result = (Double) obj;
  ...
  /**
   * Generate an appropriate string to use in an SQL insert statement for a VARCHAR colummn, taking
   * into account NULL strings and strings with embedded quotes.
   *
   * @param original String to transform
   * @return NULL if null string, otherwise massaged string with doubled quotes for SQL
   */
  ...
  /**
   * Generates an appropriate number format string for doubles etc to be used to include a number in
  ...
  /**
   * Generates an appropriate number format string for doubles etc. to be used to include a number
   ...
   *
   * <p>Specific databases will represent this with their own particular physical type, for example
   * "TINYINT(1)", "BOOLEAN" or "BIT"; see {@link #toPhysical(mondrian.spi.Dialect)}.
   */
