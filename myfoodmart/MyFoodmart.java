...
import ru.yandex.clickhouse.except.*;
 ...
 */
public class MyFoodmart {
    // Constants

    private static final Logger LOGGER =
        Logger.getLogger(MyFoodmart.class);
    ...
    private void load() throws Exception {
        ...
        generateUniqueConstraints = false;
    ...
    private String columnValue(ResultSet rs, Column column) throws Exception {
         ...
         * in the result set
         */
        } else if (columnType == Type.Bigint) {
            if (obj.getClass() == Double.class) {
                try {
                    Double result = (Double) obj;
                    return integerFormatter.format(result.doubleValue());
         ...
         */
        } else if (columnType.name.startsWith("VARCHAR")) {
         ...
         */
             ...
             */
            } else if (columnType == Type.Currency) {
                final Matcher matcher =
                    decimalDataTypeRegex.matcher(column.typeName);
                if (!matcher.matches()) {
                    throw new Exception(
        ...
        throws Exception
    {
        ...
        if (columnType == Type.Timestamp) {
            ...
            int dotIdx = columnValue.indexOf(".");
            if (dotIdx > 0) {
                return columnValue.substring(0, dotIdx) + "'";
            } else {
                return columnValue;
            }
